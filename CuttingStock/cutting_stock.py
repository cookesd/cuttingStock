...
from .classes.CuttingStockSolution import CuttingStockSolution

def cuttingStockProblem(cs,show = True, graph = True):
...
#        colorKeys = list(resultDF.drop('patternQuantity').index)
...
#                    bottomVals = list(plotDF[plotDF.columns[i]])
    ...
    lenDict = {key:value['reqQuant'] for key,value in cs.cuts.items()}
    boardLength = cs.boardLength
    epsilon = .005
